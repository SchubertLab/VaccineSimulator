...
for k in range(0,7):
    ...
    threshold = 0.5  # Define a threshold for response
    y_s1_test = (y_s1_test > threshold).astype(int)  # Convert to binary labels
    y_s1_train = (y_s1_train > threshold).astype(int)  # Convert to binary labels

    y_t1_test = (y_t1_test > threshold).astype(int)  # Convert to binary labels
    y_t1_train = (y_t1_train > threshold).astype(int)  # Convert to binary labels


    # Compute counts for training data
    agg_high = (y_s1_train == 1).sum()  # Count of high IFN response in train set
    agg_low  = (y_s1_train == 0).sum()  # Count of low IFN response in train set

    # Compute counts for test data
    cell_high = (y_s1_test == 1).sum()  # Count of high IFN response in test set
    cell_low  = (y_s1_test == 0).sum()  # Count of low IFN response in test set

    # Prepare table data
    data = [
        ["Train", agg_high, agg_low],
        ["Test", cell_high, cell_low]
    ]
    ...
    if gbr:
        ...
        gbr_t1.fit(X_train, y_t1_train)
        y_t1_pred = gbr_t1.predict(X_test)

    accuracy = accuracy_score(y_s1_test, y_s1_pred)
    precision = precision_score(y_s1_test,  y_s1_pred)
    recall = recall_score(y_s1_test,  y_s1_pred)
